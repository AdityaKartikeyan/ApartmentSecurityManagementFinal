...
class ListVehicleComponent extends Component {
  ...
  baseEntity(vehicleNo) {
    this.props.history.push(`/getByPk3/${vehicleNo}`);
  }
  ...
  viewVehicle(vehicleNo) {
    this.props.history.push(`/getByVehicleId/${vehicleNo}`);
  }
  deleteVehicle(id) {
            ...
            <tbody>
              {this.state.vehicle.map((vehicle) => (
                <tr key={vehicle.vehicleNo}>
                  ...
                  <td>
                    <button
                      onClick={() => this.editVehicle(vehicle.vehicleNo)}
                    ...
                    <button
                      style={{ marginLeft: "10px" }}
                      onClick={() => this.viewVehicle(vehicle.vehicleNo)}
