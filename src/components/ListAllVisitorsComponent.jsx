...
class ListAllVisitorsComponent extends Component {
  ...
  baseEntity(visitorId) {
    this.props.history.push(`/getByPk2/${visitorId}`);
  }
  ...
  viewVisitor(visitorId) {
    this.props.history.push(`/getByVisitorId/${visitorId}`);
  }

  deleteVisitor(id) {
    VisitorService.deleteVisitor(id).then((res) => {
      this.setState({
        visitors: this.state.visitors.filter((visitors) => visitors.id !== id),
      });
    });
  }
  ...
  render() {
    return (
      <div>
        ...
        <div className="row">
          <button className="btn btn-primary" onClick={this.addVisitor}>
            Add Visitor
          </button>
        </div>
        <br></br>
            ...
            <tbody>
              {this.state.visitors.map((visitors) => (
                <tr key={visitors.visitorId}>
                  ...
                  <td>
                    <button
                      onClick={() => this.editVisitor(visitors.visitorId)}
                    ...
                    <button
                      ...
                      onClick={() => this.deleteVisitor(visitors.id)}
                    ...
                    <button
                      style={{ marginLeft: "10px" }}
                      onClick={() => this.viewVisitor(visitors.visitorId)}
                    ...
                    <button
                      style={{ marginLeft: "10px" }}
                      onClick={() => this.baseEntity(visitors.visitorId)}
