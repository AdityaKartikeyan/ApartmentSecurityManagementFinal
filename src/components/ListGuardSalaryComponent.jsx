...
class ListGuardSalaryComponent extends Component {
  ...
  baseEntity(userId) {
    this.props.history.push(`/getByPk7/${userId}`);
  }
  ...
  viewGuardSalary(userId) {
    this.props.history.push(`/getByGuardSalaryId/${userId}`);
  }
  ...
  render() {
    return (
      <div>
        ...
        <div className="row">
          <button className="btn btn-primary" onClick={this.addGuardSalary}>
            {" "}
            Add Guard Detail
          </button>
        </div>
            ...
            <tbody>
              {this.state.guardSalary.map((guardSalary) => (
                <tr key={guardSalary.userId}>
                  ...
                  <td>
                    <button
                      onClick={() => this.editGuardSalary(guardSalary.userId)}
                    ...
                    <button
                      style={{ marginLeft: "10px" }}
                      onClick={() => this.viewGuardSalary(guardSalary.userId)}
